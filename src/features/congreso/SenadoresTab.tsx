...
export default function SenadoresTab() {
  ...
  const filteredCandidatos = SENADORES.filter((c: Candidato) => {
    ...
    const matchesTipo =
      ...
      (c.tipoSenador && selectedTipo.includes(c.tipoSenador))
    return matchesSearch && matchesParty && matchesRegion && matchesTipo
  })
  ...
  const tipoOptions = [
    { value: 'nacional', label: 'Lista Nacional' },
    { value: 'regional', label: 'Lista Regional' },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-lg border p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h2 className="text-2xl font-semibold">Buscar Senadores</h2>
            <p className="text-primary mt-1 text-sm">
              60 senadores: 30 por lista nacional y 30 por lista regional
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-4">
          <div>
            <label
              htmlFor="search-input-senadores"
          ...
          <div>
            <label
              htmlFor="party-select-senadores"
              className="mb-2 block text-sm font-medium"
            >
              Partido político
            </label>
            <Combobox
              id="party-select-senadores"
          ...
          <div>
            <label
              htmlFor="tipo-select-senadores"
              className="mb-2 block text-sm font-medium"
            >
              Tipo de senador
            </label>
            <Combobox
              id="tipo-select-senadores"
              options={tipoOptions}
              value={selectedTipo}
              onValueChange={(value) => setSelectedTipo(value as string[])}
          ...
          <div>
            <label
              htmlFor="region-select-senadores"
              className="mb-2 block text-sm font-medium"
            >
              Región
            </label>
            <Combobox
              id="region-select-senadores"
      ...
      <div className="bg-muted/30 rounded-lg border border-dashed p-8 text-center">
        <p className="text-muted-foreground">
          Estamos trabajando para traerte la lista completa de senadores. En la
          versión final podrás:
        </p>
        <ul className="text-muted-foreground mt-3 space-y-1 text-sm">
          <li>✓ Filtrar por partido político, tipo de senador y región</li>
          <li>✓ Buscar por nombre de candidato</li>
          <li>✓ Ver el perfil detallado y su historial político</li>
          <li>✓ Comparar hasta 3 senadores</li>
        </ul>
      </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">
                    {selectedCandidatos.length} senador
                    {selectedCandidatos.length > 1 ? 'es' : ''} seleccionado
        ...
        <DialogContent className="max-h-[90vh] max-w-6xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Comparación de Senadores
            </DialogTitle>
            <DialogDescription>
              Compara los perfiles, experiencia y propuestas de los senadores
              seleccionados
            </DialogDescription>
          </DialogHeader>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {candidato.tipoSenador === 'nacional'
                          ? 'Lista Nacional'
                          : `Lista Regional - ${candidato.region}`}
                      </p>
                    </div>
